...
const includeAutoGenerated = ref(route.query.type !== "user");
...
onMounted(() => {
  ...
  if (q) {
    searchForUsers(searchPage.value, false);
  }
});
...
const searchForUsers = async (page: number = 1, reset: boolean = false) => {
  ...
  // Persist search term, filter, and page in URL so back/forward/reload restores the search
  await router.replace({
    path: route.path,
    query: {
      ...route.query,
  ...
  const filterParam = !includeAutoGenerated.value ? "&type=user" : "";

  await $fetch(
    `/api/search/au?q=${encodeURIComponent(searchTerm.value)}&page=${page}${filterParam}`,
  )
    .then(
      (response: {
        users: SearchResult[];
      ...
      <UPageBody>
        <div class="w-full space-y-5">
          <div class="flex flex-col gap-3">
            ...
            <UCheckbox
              v-model="includeAutoGenerated"
              label="Include auto-generated profiles"
              :disabled="!searchTerm.trim()"
              size="sm"
            />
          </div>
